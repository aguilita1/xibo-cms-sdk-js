 ...
 * 3. Run: npm run build
 * 4. Run: npx ts-node examples/basic-usage.ts
 */

// Load environment variables from .env file
import 'dotenv/config';
import { XiboClient, XiboConfig, Context } from '../src/index';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const context: Context = client.createContext({
      timeout: 10000,
      retryAttempts: 2,
    ...
    const tokenInfo = await client.getAuthStatus();
    ...
    console.log('✨ Client disposed successfully');
  }
}
